...
\usepackage{amsmath}
\usepackage{color}
\usepackage[numbers,sort&compress]{natbib} % kills arxiv ?
\usepackage{framed}
%\usepackage{cite}
\usepackage{tikz}
\usepackage{tikz-cd}
...
\dotheorem{\numitem{thmDecomposition}}{\it (Decomposition)}
Let $G$ be an $n$-qubit 
...
% See section 2.8 of PhD thesis:
%The following lemma is used in section 2.8 below. \todo{FIX FIX FIX XXX}
%
%%\dolemma{2.5}
%\dolemma{\numitem{lemma25}}
%Given a 
...
%\doproposition{2.7}
\doproposition{\numitem{prop27}}
For a CSS gauge code Hamiltonian $H$,
\begin{align*}
\lambda_1(\Ham_{t_X,0}) &> 
...
We also apply the qualifiers self-dual and weakly self-dual
to the Hamiltonian built from such a CSS gauge code.
...
$$
    \min \Bigl\{ \lambda_1(H_{0,0}) - \lambda_2(H_{0,0}),\ 
        \min_{\substack{t_X\in\Span{T_X}\\t_X\ne 0} }
         \{ \lambda_1(H_{0,0}) - \lambda_1(H_{t_X,0}) \} \Bigr\}.
$$
\doproof
...
\todo{
This theorem says that the gap of a weakly self-dual
Hamiltonian comes from the $H_{0,0}$ sector,
or otherwise a frustrated $X$-type stabilizer.
In all the examples we consider in section \ref{SecNumerical}, it 
turns out to be the later.}
