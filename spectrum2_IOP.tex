...
given by \begin{eqnarray*}\mathrm{Orbit}(\ket{000}) = \{\ket{000}, \ket{011}, \ket{110}, \ket{101}\}.\end{eqnarray*}
Notice that the $ZZZ$ term fixes all these states.
Summing over this orbit we get the following ground state:
\begin{eqnarray*}\mathrm{GS} = \ket{000}+\ket{011}+\ket{110}+\ket{101}.\end{eqnarray*}
...
generating set 
\begin{eqnarray*}\{\omega , X_1, .., X_n, Z_1, .., Z_n\}\end{eqnarray*}
...
\begin{eqnarray*} G := \langle G_0\rangle.\end{eqnarray*}
...
\begin{eqnarray*}G = \Stab\times R,\end{eqnarray*}
where $R\cong \Pauli_r$ for some $1\le r\le n,$
and $\Stab\cong \Z_2^{m}$ for $0\le m<n.$
Therefore, 
\begin{eqnarray*}|G| = |\Stab| |R| = 2^{m+2r+1}.\end{eqnarray*}
...
defined on the independent generators as:
    \begin{eqnarray*} \rho(X_i) = \pm 1,\quad \rho(Z_i) = \pm 1.\end{eqnarray*}
...
\begin{eqnarray*} \Ham := \sum_{g\in G_0} \rho_{\mathrm{pauli}}(g).\end{eqnarray*}
...
it as 
\begin{eqnarray*}\Span{A} = \{ vA | v \in \Fmd \}.\end{eqnarray*}
...
\begin{eqnarray*}\Fnd = \Span{S_Z} \oplus V\end{eqnarray*}
...
%\newcommand\mystack[2]{\genfrac{}{}{0pt}{}{#1}{#2}} % fail
%\newcommand\mystack[2]{{#1\above 0pt #2}} % too small
%\newcommand\mystack[2]{{#1\atop #2}} % too small
\newcommand\mystack[2]{{#1,\thinspace #2}} % Brrrr...
...
\fl\ \ \ \ %
H_{0,0} = \sum_{\mystack{v\in\Frd}{g_X\in G_X } }
...
H_{t_X,0} = \!\sum_{\mystack{v\in\Frd}{g_X\in G_X } }
  \!\!\bket{v+g_X  R_Z^\top}\bbra{v} 
  + \sum_{v\in\Frd} \bigl(
...
H_{t_X,t_Z} =\!\!\! \sum_{\mystack{v\in\Frd}{g_X\in G_X } }
   \!\!\!\! \eta(t_Z g_X^\top)
...
%    \substack{t_X\in\Span{T_X}\\l_X\in\Span{L_X}}}
    \mystack{t_X\in\Span{T_X}}{l_X\in\Span{L_X}}}
        \Gamma_{t_X}.
\end{eqnarray*}
...
The vertices
\begin{eqnarray*}\Bigl\{ \bket{t_X + l_X} \Bigr\}_{t_X\in\Span{T_X},\ l_X\in\Span{L_X}}\end{eqnarray*}
...
we have:
\begin{eqnarray*}\lambda_1(\Ham) = \lambda_1(\Ham_{0,0})\end{eqnarray*}
...
gauge generators are equal: \begin{eqnarray*}G_X = G_Z.\end{eqnarray*}
...
%\begin{eqnarray*}|G_0|=2l^2,\ |R_0| = 2(l-1)^2,\ |\Stab_0| = 2(l-1).\end{eqnarray*}
...
G_0 = &\{X_i X_{i+1}, Z_i Z_{i+1}\  \smbox{for} i=1,...,n\} \\
    \thinspace \ \ \ \ = &\{X_i X_{i+1}, Z_{i+1} Z_{i+2}\ \smbox{for}  i=1,3,..,n-1\}\\
