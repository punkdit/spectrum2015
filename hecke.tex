...
An \Defn{incidence system} is a triple 
$\Gamma=(X,*,\tau)$ where $X$ is a set of \Defn{elements}
with a symmetric, reflexive \Defn{incidence relation} $*$
and $\tau$ is a map $\tau:X\to I$ where $I$ is a set of \Defn{types}
such that distinct $x,y\in X$ with $x*y$ satisfy $\tau(x)\ne\tau(y).$
The \Defn{rank} of $\Gamma$ is the cardinality of $I.$

A \Defn{flag} of $\Gamma$ is a set of mutually incident
elements of $\Gamma.$
Flags of type $I$ are called \Defn{chambers.}

A \Defn{geometry} is an incidence system where every
maximal flag is a chamber.

A geometry is called \Defn{thin} when every
...
the \Defn{residue} $\Gamma_F$ is the 
...
residue $\Gamma_x$ is called the \Defn{vertex figure.} 
This is the subsystem of edges and faces incident on $x.$

% 
% page 73: diagram over I
...
$A_n : \mathfrak{sl}_{n+1}$ traceless $(n+1)\times (n+1)$ matrices. 
Algebra dimension is $n^2 + 2n.$
Number of roots is $n(n+1).$

$B_n : \mathfrak{so}_{2n+1}$.
Algebra dimension is $2n^2 + n.$
