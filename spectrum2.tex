...
a quantum memory we would expect a spectral gap between the
...
We find that the gauge color code Hamiltonian
decomposes into six mutually commuting ideals in Section \ref{Sec45}.
...
The paper concludes with Section \ref{Sec7} where
...
\section{Some motivating examples}\label{Sec2}
...
Given a CSS gauge group $G$ with generators $G_X\cup G_Z$
we can find a solution to the following $\Field$-linear
...
model, which we call the \emph{2D $XY$-model}.
Qubits live on a 2D periodic lattice of $l\times l$ sites.
This is a self-dual model, with 
...
$$
    G_0 = \{
        ...
        \ \ \mbox{for}\ \ i,j=1,...,l \}.
$$
For $l$ even, we have $2l-3$ stabilizer generators of $X$ type:
$$
...
In a similar vein,
for the 2D $XY$-model (with $l$ even)
...
as well as the 2D $XY$-model and gauge color code model.
...
of the 3D gauge color code models of sizes $n=15,65,175.$
...
The gap of the 3D gauge color code is clearly more
...
gapless behaviour that the gauge color code  may
be gapped.
