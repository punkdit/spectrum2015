...
The paper concludes with Section 2.11 where
...
\underline{\bf Example \ritem{Ex1}.}
...
\underline{\bf Example \ritem{Ex2}.}
...
\underline{\bf Example \ritem{Ex3}.}
...
Lemma \ref{lemma211} and Theorem \ref{thm212} below appear to be new results.
...
%Let $\auto$ be the group of \emph{graph automorphisms} of $\Gamma:$
%$$
%    \auto := \mbox{Aut}(\Gamma) =
%        \bigl\{ P\ \bigl|\ \Gamma P = P \Gamma,\ 
%        \mbox{where $P$ is a permutaion matrix} \bigr\}.
%$$

An \emph{$\auto$-invariant} vector $\ket{v}\in\Complex[V]$
...
In the next lemma we make use of an explicit formula
for the orbigraph.
%for the linear operators $C$ and $D.$
...
\begin{equation}\label{eqforD}
    %D = \sum_{i\in V/\auto} \ket{i}\bra{v_i}
    D = \sum_{a\in V/\auto} \ket{a}\bra{g(a)},
\end{equation}
%where $v_i\in i$ for each $i\in V/\auto$.
...
%\dolemma{2.11}
\dolemma{\ritem{lemma211}}
The orbigraph of an irreducible stoquastic matrix is also
...
%    Let $i\in V/\auto$ be an $\auto$-orbit.
%    Then $C\ket{i}$ is $\auto$-invariant
%    and so $\Gamma C\ket{i}$ is also $\auto$-invariant.
%    Let $\ket{u}\in\Complex[V/A]$ be such that
%    $$
%    \Gamma C\ket{i} = C\ket{u}.
%    $$
%    Now we let $j\in V/\auto$ and compute
%    \begin{align*}
%    \braket{j}{u} 
%        &= \bra{j} D C \ket{u} \\
%        &= \bra{j} D \Gamma C \ket{i} \\
%        &= \bra{j} D \Gamma \sum_{v\in i} \ket{v} \ \ \ \mbox{using eq. \Eref{eqforC}}\\
%        &= \sum_{v\in i} \bra{v'} \Gamma \ket{v} \ \ \ \mbox{using eq. \Eref{eqforD}, with $v'\in j$.}\\
%        %&\ge 0 \ \ \ \mbox{if $i\ne j$.}
%    \end{align*}
%    And so $\braket{j}{u}\ge 0$ if $i\ne j$ because $\Gamma$ is stoquastic.
We first show that the orbigraph of $\Gamma$ is stoquastic.
Let $a, b\in V/\auto$, be distinct $\auto$-orbits. We use equation \Eref{eqforOrbi}:
\begin{align*}
...
%\dotheorem{2.12}
\dotheorem{\ritem{thm212}}
%The spectrum of the orbigraph of $\Ham_{t_X,0}$ contains the top eigenvalue of $\Ham_{t_X,0}.$
...
\begin{align*}
    ...
    &= \lambda \ket{u}
\end{align*}
%where we used the fact that $D$ is a left inverse for $C$.
...
%Graph automorphisms act by permuting vertices, which
%correspond to symmetry invariant basis vectors for $\Ham_{t_X,0}.$
%By Lemma 2.5, the top eigenvector for this operator is
%positive and non-degenerate.
%This eigenvector must be fixed by
%every graph automorphism
%of $\Ham_{t_X,0}$.
%$F$ because $F$ commutes with $\Ham_{t_X,0}.$
\tombstone
...
%is the same as the automorphism group 
%$Aut(G)$ of the gauge group.
%In general it is true that $Aut(G) \le \auto.$

%We can also write $H/\auto = EHF$ using the following two matrices for $F$ and $E:$
%See also \cite{Cvetkovic1980} chapter 5.


For the remainder of this section 2.8,
we will be applying Theorem \ref{thm212} to the
...
The rows without $t_X$ specified correspond to a choice of $t_X\ne 0$ 
...
%
%Can you hear the sound of a semi-simple Lie algebra?

% The spin transform for the symmetry invariant basis involves
% non-local spin operators, and results in another gauge code
% Hamiltonian (in general with fewer qubits) for each block. 
% Is there anything further to be learned from examining these spin transforms,
% or the resulting Hamiltonian operators?
...
We now use this representation theory 
