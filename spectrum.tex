...
%From equation \Eref{hamblockgs} we see that 
%$H_{0,0}$ is also irreducible stoquastic 
...
%Such an eigenvector will have entries that
...
%It follows that the off-diagonal matrix entries of $\Ham/\auto$,
%indexed by a pair of $\auto-$orbits $i$ and $j$,
%$i\ne j$,
%are defined by
%$$
%    (\Ham/\auto)_{ij} = \bigl|\bigl\{ g\in G \smbox{s.t.} gv \in j \bigr\}\bigr| \smbox{where}v\in i.
%$$
%In other words, 
%$(\Ham/\auto)_{ij}$ with $i\ne j,$ counts the number of gauge group
...
%We will apply this orbigraph construction
%to each of the blocks $H_{t_X,0}$.
%From Lemma 2.5 above, and using the fact that we are
