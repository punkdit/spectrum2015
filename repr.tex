...
by doing some kind of ad-hoc, unmotivated, spin transformation.
...
Next, to find the cosets of $G$ in $\Pauli_n$ we take
the group closure of $G-\Pauli_n$. This is another
...
%$$
%    %\rho_t(h)=\pm I^{\otimes 2^r}\ \mbox{for}\ h\in H_0,
%    %\rho_t(h)= [t,h] = th^{-1}th^{-1} = \pm I^{\otimes 2^r}\ \mbox{for}\ h\in H_0,
...
an operator $\Ham:\Complex[n]\to\Complex[n]$ and
normally defined
...
%
%In general we can represent $G=H\times R$ in $R$ itself..
