...
\title{Representations and Spectra of Subsystem Code Hamiltonians}
...
the abelian group $T$ and multiplicity indexed by logical operators $l\in L:$
$$
    \Ham =  \bigoplus_{l\in L, t\in T} \Ham_t
$$
\end{framed}
...
%\noindent{\bf Fact 1:}
%
%The blocks $H_t$ of a CSS Hamiltonian are also CSS.
%\end{framed}
%
...
%\subsubsection{Self-representations}
%
...
\section{Spectra}
\label{spectra}
\subsection{Perron-Frobenius theory}
...
We will call this a \emph{CSS Hamiltonian} after the stabilizer codes
of the same name.
...
breaks into $2^k$ irreducible matrix blocks all of which are equal.
...
Each block $\Ham_t$ has non-degenerate groundspace.
\end{framed}
\emph{!! check what happens with weights $\pm 1.$}
...
\subsection{The Orbi-Hamiltonian}
...
The orbi-Hamiltonian of $\Ham_I$ contains the ground eigenvalue of $\Ham_I.$
\end{framed}
...
% This time $Aut(G) = A$ ???
The orbi-Hamiltonian method can be applied in the case
where the Hamiltonian weights for $X$ and $Z$ are uniform as $w_X$ and $w_Z.$
We separate the $X$ and $Z$ terms of the orbi-Hamiltonian to show
how this works:
$$
\Ham_I//\auto = 
...
we get the spectrum of $\Ham_I$ and note that the orbi-Hamiltonian lifts
...
If we wanted to capture these other spaces we could construct
a different orbi-Hamiltonian by 
summing over the orbit using different characters
(or \emph{momenta} using abelian terminology).
...
Numerics give the full spectrum of $\Ham_I$ and we note that the orbi-Hamiltonian lifts
