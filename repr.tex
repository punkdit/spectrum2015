...
gauge group in the $t=0$ irrep is to send
$v\in R_X$ to 
$v + g_X P_X R_Z^\top.$
...
We have the following off-diagonal components:
$$
%    H_{0,0}(v, v+g_X P_X R_Z^\top) = 1,\ \ \mbox{for}\ g_X\in G_X, v\in \Fr
    \bra{v} H_{0,0} \ket{v+g_X P_X R_Z^\top} = 1,\ \ \mbox{for}\ g_X\in G_X, v\in \Fr
$$
...
we have:
$$
    \bra{v} H_{0,0} \ket{v} = |G_Z| - 2w(G_Z R_X^\top v^\top),
$$
for $v\in \Fr.$
...
and so $$\lambda_1(\Ham_{0,0}) = \lambda_1(\Ham)$$
and for $t\ne I:$
$$
