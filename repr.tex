...
have in summary,
$$
H_{0,0} = \sum_{g_X\in G_X, v\in\Fr} 
...
$$
H_{t_X,0} = \sum_{g_X\in G_X, v\in\Fr} 
  \ket{v+g_X P_X R_Z^\top}\bra{v} 
  + \sum_{v\in\Fr} \bigl(
...
\begin{align*}
H_{t_X,t_Z} = &\sum_{g_X\in G_X, v\in\Fr} 
    \eta(t_Z g_X^\top)
  \ \ket{v+g_X P_X R_Z^\top}\bra{v} \\
  &+ \sum_{v\in\Fr} \bigl(
    |G_Z| - 2w(G_Z R_X^\top v^\top + G_Z t_X^\top)
...
\begin{framed}
In the computational basis, a CSS gauge code
Hamiltonian is the direct sum of $2^{m_Z}$
irreducible
matrices $\Gamma_{t_X}$
