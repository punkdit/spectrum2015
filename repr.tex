...
In the remainder of this paper, we restrict our
attention to Hamiltonians formed from terms 
...
$$
    \Pauli^X_n \cong F^n_2,  \ \ 
    \Pauli^Z_n \cong F^n_2. 
$$
we do this in the obvious way by sending $X_i$ to the basis vector with
$1$ in the $i-$th entry, and similarly for each $Z_i$. 
We also identify the computational basis of our statespace $\Complex[n]$
with $F^n_2$ in the obvious way.
...
$$
    g_X \in \Pauli^X_n \cong F^n_2, \ \ g_X : v \longmapsto g_X + v
$$

$Z$-type operators act on basis vectors using $F_2$ inner product:
$$
    g_Z \in \Pauli^Z_n \cong F^n_2, \ \ g_Z : v \longmapsto g_Z v^\top
$$
This is a $F_2$ scalar, just zero or one. We think of this
as a ``syndrome''.
...
Now we wish to use this language to decompose the gauge group $G.$
