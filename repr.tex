...
What is an operator? From the perspective of a quantum code these are the
things that we use to diagnose errors and perform error correction. We can
also interpret these operators as the terms of Hamiltonian, whose
groundspace corresponds to the energetically protected codespace.
From the mathematical perspective we examine three different motions of
representation theories from these operators, with the view to extracting
spectral information about Hamiltonian. Group theory representations give
the block diagonalisation of the Hamiltonian as labeled by stablisors. The
coarser tool of Perron-Frobenius theory gives information about the
spectral layout of these blocks in the case of CSS gauge codes. At the
finest level, the operators in each of these blocks form a semisimple Lie
algebra and ideals in this algebra correspond to tensor product of
representations. This allows us to perform exact diagonalisation on some
large instances of the gauge colour code Hamiltonian. These numerics
support the conjecture that these Hamiltonians are gapped.
\end{abstract}
...
We now apply the tools built so far in order
...
eigenspaces are labelled by what we called syndromes previously,
and these are all one dimensional because the kernel of $R_Z$ is trivial.
...
Each instance is labelled by the number of vertices $n$.
\begin{center}
...
%Every interior edge is in the boundary of three balls.
%Edges contained in the regions are either in the boundary of two balls or
%otherwise are contained in one of the borders and the boundary of one ball.
...
%We color each face with the two colors of the balls 
...
We now turn to a coarser notion of reducibility.
...
$$
    \ket{v} \mapsto g_X \ket{v}, \smbox{for all} v\in\Fn, g_X\in G_X.
$$
These are undirected edges because $g_X^2 = I.$
We also add weighted loops corresponding to the $Z$-type gauge operators:
$$
    \ket{v} \mapsto \sum_{g_Z\in G_Z} g_Z \ket{v}, \smbox{for all} v\in\Fn.
$$
...
The numerics for the compass model have been previously found \cite{Brzezicki2013}
...
\hline
gauge color & 175 & 0         &  &  267.197576  & 264.250644  &  & \\
& & & 8  & & 263.171190 & &    \\
& & & 8  & & 263.324858 & &    \\
& & & 8  & & 263.340832 & &    \\
& & & 12 & &  264.269635 & &    \\
...
that the gap tends to zero as the lattice size grows.
