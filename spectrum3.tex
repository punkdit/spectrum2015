...
as the energetic terms of a Hamiltonian model,
or as the check operators in an error correcting code.
In this case we have extensively many integrals of motion.
...
As a Hamiltonian, we are protecting information in the groundspace. 
When the terms come from a \emph{stabilizer} code
they all commute and so 
we can readily diagonalize the Hamiltonian.
Such models, for example the toric code\cite{Dennis2002}
are easily seen to be gapped.
For more general \emph{subsystem} or \emph{gauge codes}, this is
not the case, but we can still block diagonalize the Hamiltonian
with blocks labelled by eigenvalues of the 
stabilizers~\cite{Bacon2006quantum,Yoshida2010,Brzezicki2013}.
...
When the quantum code is restricted to the class of
CSS codes\cite{Calderbank1997}, the corresponding Hamiltonian
will be stoquastic. 
This means the off-diagonal terms in the Hamiltonian are
non-negative.
In this case we can use the tools from
Perron-Frobenius theory~\cite{Perron1907, Frobenius1912, Baez2012}.
%constrains the spectrum of the Hamiltonian blocks.
%with particular attention given to the spectral
%gap of the Hamiltonian.
These are new results, presented in Section \ref{Sec5},
which show where the groundstate and first excited
states are located among the blocks of the Hamiltonian.
%These results enable to numerically find 
%and are crucial for obtaining the numerical results presented in
%Section \ref{Sec6}.
In \ref{Sec6} we use these results to numerically find 
spectral gap of several gauge code Hamiltonians.
...
decomposes into six mutually commuting pieces in Section \ref{Sec45}.
...
These numerics show some evidence of a spectral gap,
which is contrasted against models that are known
...
We also apply these techniques to a new model
we call the 2D $XY$-plaquette model,
which has weight four gauge operators.

We conjecture that the decisive feature that makes
the gauge code model gapped is the lack of unbounded stabilizers.
In other words, if the excitations, or ``quasi-particles'' of
the system are localized, then we have a gap.
This fits nicely with the proposed classification of 
gapped phases of matter at long distances by topological
quantum field theories~\cite{Kitaev2003,Kapustin2017}.
%The size of stabilizers is the 
The gap of this model would also likely be a pre-requisite for
it being a quantum memory. 
The existance of quantum memories (in three dimensions)
is unknown at this time~\cite{Brown2016}.
...
%using the $2n+1$ element generating set 
%$\{X_1, .., X_n, Z_1, .., Z_n\}$
as the group generated by $n$-fold tensor products of the matrices:
$$
...
We call these \emph{CSS gauge codes}\cite{Calderbank1997}.
...
\begin{center}
\includegraphics[width=0.22\columnwidth]{pic-gcolor-1.pdf}\ \ \ 
\includegraphics[width=0.22\columnwidth]{pic-gcolor-15.pdf}\ \ \ 
\includegraphics[width=0.22\columnwidth]{pic-gcolor-2.pdf}\ \ \ 
\includegraphics[width=0.22\columnwidth]{pic-gcolor-3.pdf}
\end{center}
...
Note that the Decomposition equation \Eref{lstr} is equivalent to  
this presentation.
...
%\subsection{Proof of $(L,S,T,R)$-Decomposition theorem}\label{Sec36}
